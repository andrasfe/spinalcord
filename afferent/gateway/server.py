# mypy: ignore-errors
# ruff: noqa
# Mirrored verbatim from the live terminaleyes/raspi deployment (which
# drifted ahead of the original port: BT reconnect watchdog, multi-click,
# move_large). Lint cleanup deferred.
...
the afferent client (running on another machine) and the target machine
...
from afferent.gateway.hid_writer import HidWriteError, HidWriter, MouseHidWriter
...
def create_app(
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        if app.state.bt_hid is None and enable_bt_hid:
            try:
                from afferent.gateway.bt_hid import (
                    BluetoothHidServer,
                ...
                # AFFERENT_ name preferred; the older HANDSNEYES_ /
                # TERMINALEYES_ names are honoured too so a pre-rename
                # unit file's Environment= line keeps working.
                BT_RECONNECT_INTERVAL_S = float(
                    os.environ.get(
                        "AFFERENT_BT_RECONNECT_INTERVAL_S",
                        os.environ.get(
                            "HANDSNEYES_BT_RECONNECT_INTERVAL_S",
                            os.environ.get(
                                "TERMINALEYES_BT_RECONNECT_INTERVAL_S",
                                "20",
                            ),
                        ),
                    )
